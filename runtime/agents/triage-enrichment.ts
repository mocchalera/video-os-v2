...
import type { Candidate, CreativeBrief, QualityGateThresholds, SelectsCandidates } from "../artifacts/types.js";
import { applyQualityGateToSelects } from "../editorial/quality-gate.js";
import { deriveSemanticClusterId } from "../editorial/clustering.js";
export {
  ...
  type FilmingTimestamp,
  type RefineClusterOptions,
} from "../editorial/clustering.js";

type PeakType = "action_peak" | "emotional_peak" | "visual_peak";

export interface EnrichSelectsOptions {
  brief?: CreativeBrief;
...
export function enrichSelectsFromAnalysis(
  ...
  if (options.applyQualityGate === false) return enriched;

  const gated = applyQualityGateToSelects(enriched, segments, options);
  const counts = gated.quality_gate?.counts;
  console.error(
    `[triage:quality-gate] decisions reject=${counts?.reject ?? 0} warn=${counts?.warn ?? 0} ` +
      `pass=${counts?.pass ?? 0} unmeasured=${counts?.unmeasured ?? 0}`,
  );
  return gated;
}
