...
function headlessLlmCompleter(passLabel: string): LlmCompleter | undefined {
  ...
  const model = process.env.UNIFIED_EDITORIAL_MODEL ?? DEFAULT_UNIFIED_EDITORIAL_MODEL;
  return (prompt: string) =>
    callGeminiJson(prompt, model, {
      retryLabel: passLabel,
...
async function completeJsonWithRetry(
  ...
  try {
    ...
    const second = await llm(repairPrompt(prompt, first, firstError));
...
export async function fineCutRefinement(
  ...
  return normalizeFineBlueprint(parsed, { brief, selects, blueprint, marlinEvents, bgmDurationSec });
}
