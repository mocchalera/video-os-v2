...
function checkCadenceDistribution(input: ReviewMetricsInputs): ReviewMetricCheck {
  ...
  if (Object.values(sections).some((durations) => durations.length === 0)) {
    return skipped(
      id,
      "rhythm",
      "Opening, middle, and ending sections must each have at least one V1 clip.",
    );
  }

  const averages = {
    opening: round(mean(sections.opening)),
    middle: round(mean(sections.middle)),
    ending: round(mean(sections.ending)),
  };
  const ranks = {
  ...
  const comparisons = [
    compareCadencePair("opening", "middle", ranks.opening, ranks.middle, averages.opening, averages.middle),
    compareCadencePair("middle", "ending", ranks.middle, ranks.ending, averages.middle, averages.ending),
  ...
  return check(id, "rhythm", comparisons.length > 0 ? "fail" : "pass", measured, {
    ...
    ? comparisons
    : ["Average V1 shot length order is consistent with cadence policy."]);
}
...
function checkPeakRetention(input: ReviewMetricsInputs): ReviewMetricCheck {
  ...
  const selectedSegmentIds = new Set(selectedClips.map((clip) => clip.segment_id));
  ...
  for (const segmentId of [...selectedSegmentIds].sort()) {
    const segment = segmentMap.get(segmentId);
    const strongPeaks = strongPeaksForSegment(segment, strongPeakThreshold);
    evaluatedStrongPeaks += strongPeaks.length;
    if (strongPeaks.length === 0) continue;
    const clipsForSegment = selectedClips.filter((clip) => clip.segment_id === segmentId);
    for (const peak of strongPeaks) {
      const retained = clipsForSegment.some((clip) =>
  ...
  if (evaluatedStrongPeaks === 0) {
    ...
    }, ["Selected timeline segments contain no strong peak moments to protect."]);
  }
...
function checkHookDensity(input: ReviewMetricsInputs): ReviewMetricCheck {
  ...
  return check(id, "emotion", status, {
    ...
    ? [`Hook density ${round(density)} meets target ${target}.`]
...
function checkSameAssetAdjacency(input: ReviewMetricsInputs): ReviewMetricCheck {
  ...
  return check(id, "eye_trace", violations.length > 0 ? "fail" : "pass", {
    adjacent_pairs: v1.length - 1,
    same_asset_pairs: violations,
  }, {
    max_same_asset_adjacent_pairs: 0,
    scope: "V1",
  }, violations.length > 0
...
function checkSpeechCut(input: ReviewMetricsInputs): ReviewMetricCheck {
  ...
  const clipsToCheck = getAllClips(input.timeline).filter((clip) =>
  ...
  for (const clip of clipsToCheck) {
    ...
    for (const utterance of utterances) {
      ...
      if (isBoundaryInsideUtterance(clip.src_in_us, utterance, DEFAULT_SPEECH_CUT_GUARD_US)) {
        violations.push({
          clip_id: clip.clip_id,
          asset_id: clip.asset_id,
      ...
      if (isBoundaryInsideUtterance(clip.src_out_us, utterance, DEFAULT_SPEECH_CUT_GUARD_US)) {
        violations.push({
          clip_id: clip.clip_id,
          asset_id: clip.asset_id,
          utterance_start_us: utterance.start_us,
...
function clipHasRetainedPeakSignal(
  ...
  const candidate = candidateForClip(clip, candidateMap);
  const peakStrength = candidate?.editorial_signals?.peak_strength_score ?? 0;
  const peakSignals = candidate?.peak_signals;
  const strongestCandidateSignal = Math.max(
    peakStrength,
