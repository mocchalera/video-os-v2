...
export class PremierePreflightProcessBroker {
  ...
  async #discover(
    ...
    const executable = this.#canonicalExecutable("/usr/bin/which"), argv = [binding.tool];
  ...
  async #runEvaluation(
    ...
    switch (invocation.kind) {
      ...
      case "sysctl_exact_if_reachable":
        ...
        executable = this.#canonicalExecutable("/usr/sbin/sysctl");
      ...
      case "ps_exact_if_reachable":
        ...
        executable = this.#canonicalExecutable(this.#platform === "darwin" ? "/bin/ps" : "/usr/bin/ps");
  ...
  #canonicalExecutable(value: string): string {
    if (!path.isAbsolute(value)) forbidden("executable path is not canonical absolute");
    try {
      const realpath = this.#realpath(value);
      if (!path.isAbsolute(realpath) || this.#realpath(realpath) !== realpath) {
        forbidden("executable path is not canonical absolute");
      }
      return realpath;
    } catch (error) {
