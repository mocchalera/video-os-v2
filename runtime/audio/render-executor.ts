...
import { finishDialogueAudio } from "./finish-runner.js";
import {
  masterAudio,
  measureAudioLoudness,
  type LoudnormMeasurement,
} from "./mastering.js";
import type { AudioMixReport } from "./mixer.js";
import {
  hashAudioRenderPlan,
  hashFile,
  type AudioRenderCue,
...
function measurementPlaceholder(): LoudnormMeasurement {
  return {
    input_i: "-inf",
    input_tp: "-inf",
    input_lra: "0.00",
...
export async function executeAudioRenderPlan(
  ...
  ensureOutputsAvailable(
    [rawDialoguePath, finalMixPath, reportPath],
  ...
  try {
    ...
    const premasterMeasurement = plan.final_mastering.count === 1
      ? await measureAudioLoudness(stagedPremaster, plan.final_mastering)
      : await measureAudioLoudness(stagedFinalMix, plan.final_mastering);
    const outputMeasurement = await measureAudioLoudness(
      stagedFinalMix,
    ...
    const sfxMeasurements = await Promise.all(
      sfxArtifacts.map(async ({ cue, a3Path }) => ({
        cue_id: cue.cue_id,
        measurement: await measureAudioLoudness(a3Path, plan.final_mastering),
      })),
    );
    ...
    const report: AudioMixReport = {
      ...
      final_mastering: {
        ...
        premaster_measurement: premasterMeasurement ?? measurementPlaceholder(),
