...
import { createMarlinFnFromEnvironment, marlinModelFromEnvironment } from "../pipeline/stages/marlin.js";
...
export async function runMarlinQA(
  ...
  const mockReport = options.mock ?? (
    ownsMarlinFn && marlinModelFromEnvironment(absProjectDir, options.repoRoot).inference_mode === "mock"
  );
...
export function buildMarlinQAReport(input: BuildMarlinQAReportInput): MarlinQAReport {
  ...
  const visualQA = input.mock === true ? "unverified" : "verified";
  ...
  const reportScore = input.mock === true ? 0 : scoreReport(issues);
  const overallAssessment = input.mock === true
    ? "Marlin QA was run in mock mode; visual QA is unverified and cannot be counted as a pass."
    : buildOverallAssessment(issues, emotionArc.follows_brief);
  ...
  return {
    ...
    score: reportScore,
    visual_qa: visualQA,
    ...(input.mock === true ? { mock: true, visual_qa_reason: "mock_marlin" } : {}),
  };
}
...
export function detectContinuityIssues(events: MarlinQAEvent[]): MarlinQAIssue[] {
  ...
  const seen: Array<{ index: number; event: MarlinQAEvent }> = [];

  events.forEach((event, index) => {
    const repeated = [...seen]
      .reverse()
      .map((previous) => ({ previous, evidence: continuitySceneEvidence(previous.event.description, event.description) }))
      .find(({ evidence }) => evidence !== null);

    if (repeated && repeated.evidence !== null && index - repeated.previous.index > 1 && !reported.has(repeated.evidence)) {
      reported.add(repeated.evidence);
      issues.push({
        timestamp_sec: event.start_sec,
...
export function summarizeMarlinQAReport(report: MarlinQAReport): string[] {
  ...
  const visualStatus = marlinQAStatus(report);
  const visualParts = [
    visualStatus,
    ...(report.mock === true ? ["mock=true"] : []),
    ...(report.visual_qa_reason ? [report.visual_qa_reason] : []),
  ];
  return [
    `Visual QA: ${visualParts.join(" ")}`,
    ...
    `Overall: ${isMarlinQAReportVerified(report) ? report.overall_assessment : "Visual QA is not verified; do not count this report as a pass."}`,
  ];
}
...
const STRONG_CONTINUITY_PHRASE_TOKENS = 4;
const STRONG_CONTINUITY_TOKEN_JACCARD = 0.75;
...
function continuitySceneEvidence(leftDescription: string, rightDescription: string): string | null {
  if (isGenericContinuityDescription(leftDescription) || isGenericContinuityDescription(rightDescription)) return null;

  const leftExact = normalizeGenericContinuityDescription(leftDescription);
  const rightExact = normalizeGenericContinuityDescription(rightDescription);
  if (leftExact === rightExact) return `exact:${leftExact}`;

  const leftTokens = meaningfulTokens(leftDescription);
  const rightTokens = meaningfulTokens(rightDescription);
  const leftPhrases = new Set(strongContinuityPhrases(leftTokens));
  const sharedPhrase = strongContinuityPhrases(rightTokens)
    .find((phrase) => leftPhrases.has(phrase));
  if (!sharedPhrase || tokenJaccard(leftTokens, rightTokens) < STRONG_CONTINUITY_TOKEN_JACCARD) {
    return null;
  }
  return `multi:${sharedPhrase}`;
}

function strongContinuityPhrases(tokens: string[]): string[] {
  const phraseLength = STRONG_CONTINUITY_PHRASE_TOKENS;
  if (tokens.length < phraseLength) return [];

  return tokens.slice(0, tokens.length - phraseLength + 1)
    .map((_, index) => tokens.slice(index, index + phraseLength).join(" "));
}

function tokenJaccard(leftTokens: string[], rightTokens: string[]): number {
