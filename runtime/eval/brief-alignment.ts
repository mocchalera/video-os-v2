...
import {
  ...
  runBriefAlignmentJudgeWithRuntime,
...
import {
  ...
  type BriefAlignmentCompositeJudgeSource,
  type StageResult,
...
async function evaluateSelectsStage(
  ...
  judge: BriefAlignmentJudgeReport | null;
  decisionRuntime?: NonNullable<BriefAlignmentReport["decision_runtime"]>[number];
}> {
  ...
  const judgeResult = options.useLlm === false
    ? null
    : await runBriefAlignmentJudgeWithRuntime(
        { brief, stage: "selects", artifactYaml },
        options.judge,
      );
  const judge = judgeResult?.report ?? null;
  const result = stageResult(mergeJudge(base, judge));
  return {
    result,
    notes: [
      ...(judgeResult && !judge ? [`selects judge skipped: runtime=${judgeResult.decision_runtime.runtime}`] : []),
      ...(judge?.notes ?? []),
    ],
    mustAvoidViolated: avoid.score === 0,
    mustHaveScore: base.must_have_coverage.score,
    judge,
    ...(judgeResult ? { decisionRuntime: judgeResult.decision_runtime } : {}),
  };
}
...
async function evaluateBlueprintStage(
  ...
  return {
    result,
    notes: [
      ...(judgeResult && !judge ? [`blueprint judge skipped: runtime=${judgeResult.decision_runtime.runtime}`] : []),
      ...(judge?.notes ?? []),
    ],
    judge,
    ...(judgeResult ? { decisionRuntime: judgeResult.decision_runtime } : {}),
...
export async function evaluateBriefAlignment(
  ...
  const decisionRuntime: NonNullable<BriefAlignmentReport["decision_runtime"]> = [];
  let judgeSource: BriefAlignmentCompositeJudgeSource = "deterministic-only";
  ...
  if (stagesToRun.has("selects")) {
    ...
    if (fs.existsSync(selectsPath)) {
      ...
      if (evaluated.decisionRuntime) {
        decisionRuntime.push(evaluated.decisionRuntime);
      }
      if (evaluated.judge) {
        judgeSource = "llm-assisted";
      }
...
export function renderBriefAlignmentMarkdown(report: BriefAlignmentReport): string {
  ...
  lines.push(`- Judge source: ${report.judge_source ?? "unknown"}`);
  lines.push("");
