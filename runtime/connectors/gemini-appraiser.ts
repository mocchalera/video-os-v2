...
export const APPRAISER_CONNECTOR_VERSION = "gemini-appraiser-v1.0.0";
...
function normalizeAppraiserVisualQuality(raw: unknown): AppraiserVisualQuality {
  ...
  return {
    composition_score: clampScore(record.composition_score ?? nestedScores.composition_score),
    light_quality: clampScore(record.light_quality ?? nestedScores.light_quality),
    focus_sharpness: clampScore(
      record.focus_sharpness ??
      record.focus_score ??
      nestedScores.focus_sharpness ??
      nestedScores.focus_score,
    ),
    subject_prominence: clampScore(record.subject_prominence ?? nestedScores.subject_prominence),
  };
}
