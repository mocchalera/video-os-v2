...
// ── Source-map media path authorization ─────────────────────────────

export interface SourceMapPathEntry {
  asset_id?: string;
...
const SOURCE_MAP_API_REDACTED_ITEM_KEYS = new Set([
  "local_source_path",
  "link_path",
  "source_locator",
  "image_sequence",
...
export function redactSourceMapItemForApi(
  ...
  for (const [key, value] of Object.entries(item)) {
    if (SOURCE_MAP_API_REDACTED_ITEM_KEYS.has(key)) continue;
    redacted[key] = value;
  }
  return redacted;
}

export function redactSourceMapForApi<T extends { items?: unknown[] }>(
  sourceMap: T,
): T {
  return {
    ...sourceMap,
    items: Array.isArray(sourceMap.items)
      ? sourceMap.items.map((item) =>
          item && typeof item === "object"
            ? redactSourceMapItemForApi(item as Record<string, unknown>)
            : item,
        )
      : sourceMap.items,
  };
...
function existingRealPath(filePath: string): string | null {
  try {
...
function allowedMediaRoots(projectDir: string): string[] {
  const roots = [
    projectDir,
  ...
  for (const root of roots) {
    ...
    if (real) realRoots.add(real);
  }
  return [...realRoots];
}

function mediaRootsFromEnv(): string[] {
  const raw = process.env.VIDEO_OS_ALLOWED_MEDIA_ROOTS ?? process.env.VOS_ALLOWED_MEDIA_ROOTS;
  if (!raw) return [];
  return raw
...
function sourceMapCandidates(
  ...
  preferSourceMapDir = false,
): string[] {
  if (!rawPath) return [];
  if (path.isAbsolute(rawPath)) return [rawPath];

  const sourceMapDir = path.dirname(sourceMapPath);
  const candidates = preferSourceMapDir
    ? [path.resolve(sourceMapDir, rawPath), path.resolve(projectDir, rawPath)]
    : [path.resolve(projectDir, rawPath), path.resolve(sourceMapDir, rawPath)];
  return [...new Set(candidates)];
}

function linkedMediaRealPaths(
  projectDir: string,
  sourceMapPath: string,
  entry: SourceMapPathEntry,
): Set<string> {
  const linked = new Set<string>();
  const mediaDir = path.resolve(projectDir, "02_media");
  for (const candidate of sourceMapCandidates(projectDir, sourceMapPath, entry.link_path)) {
    const resolvedCandidate = path.resolve(candidate);
    if (!pathInside(resolvedCandidate, mediaDir)) continue;
    const real = existingRealPath(resolvedCandidate);
    if (real) linked.add(real);
  }
  return linked;
}

function isAllowedMediaRealPath(
  projectDir: string,
  realPath: string,
  exactAllowedRealPaths: Set<string>,
): boolean {
  if (exactAllowedRealPaths.has(realPath)) return true;
  return allowedMediaRoots(projectDir).some((root) => pathInside(realPath, root));
}

export function resolveAllowedSourceMapPath(
  projectDir: string,
  sourceMapPath: string,
  entry: SourceMapPathEntry,
): string | null {
  const exactAllowedRealPaths = linkedMediaRealPaths(projectDir, sourceMapPath, entry);
  const candidates = [
    ...sourceMapCandidates(projectDir, sourceMapPath, entry.local_source_path, true),
    ...sourceMapCandidates(projectDir, sourceMapPath, entry.link_path),
    ...sourceMapCandidates(projectDir, sourceMapPath, entry.source_locator, true),
  ];

  for (const candidate of candidates) {
    const real = existingRealPath(candidate);
    if (!real) continue;
    if (isAllowedMediaRealPath(projectDir, real, exactAllowedRealPaths)) {
      return candidate;
    }
  }
  return null;
