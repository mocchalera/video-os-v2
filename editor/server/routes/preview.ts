...
export function createPreviewRouter(
  ...
  router.post("/:id/preview", async (req, res) => {
    ...
    try {
      ...
      // Build RenderSpec
      const captionApprovalPath = resolveCaptionApprovalPath(projectDir);
      const captionStylePreset = resolveProjectCaptionStylePreset(projectDir);
      const resolveAssetPath = buildAssetResolver(projectDir, sourceMapPath, sourceMap);
      const renderSpec = buildRenderSpec(
        timeline,
        timelineRevision,
        resolveAssetPath,
        { captionApprovalPath, captionStylePreset },
      );
  ...
  router.get("/:id/preview/status", (req, res) => {
    ...
    try {
      ...
      if (fs.existsSync(timelinePath)) {
        ...
        const captionApprovalPath = resolveCaptionApprovalPath(projDir);
        const captionStylePreset = resolveProjectCaptionStylePreset(projDir);
        const resolveAssetPath = buildAssetResolver(projDir, sourceMapPath, sourceMap);
