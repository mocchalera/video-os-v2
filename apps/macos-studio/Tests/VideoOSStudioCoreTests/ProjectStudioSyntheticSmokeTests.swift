...
final class ProjectStudioSyntheticSmokeTests: XCTestCase {
    func testSmokePackagesSyntheticMediaAndExportsPacketWithInjectedRunners() throws {
        let repositoryRoot = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("videoos-studio-smoke-repo-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: repositoryRoot.appendingPathComponent("scripts"), withIntermediateDirectories: true)
        try "{}".write(to: repositoryRoot.appendingPathComponent("package.json"), atomically: true, encoding: .utf8)
        try "worker".write(to: repositoryRoot.appendingPathComponent("scripts/editor-job-worker.ts"), atomically: true, encoding: .utf8)

        let result = try ProjectStudioSyntheticSmoke.run(
        ...
        XCTAssertEqual(result.syntheticBuildResult.builtCount, 1)
        ...
        XCTAssertTrue(result.renderResult.succeeded)
        XCTAssertTrue(result.finalVideoExists)
        XCTAssertGreaterThanOrEqual(result.editorPacketMediaCount, 2)
        XCTAssertEqual(result.editorPacketVerificationStatus.readinessLabel, "packet verified")
        XCTAssertTrue(result.editorPacketVerificationStatus.finalMediaIncluded)
        XCTAssertTrue(result.editorPacketVerificationStatus.finalAudioIncluded)
        XCTAssertGreaterThan(result.indexStatus.documentCount, 0)
        XCTAssertEqual(result.indexSummary.audioStoryNodeCount, 1)
        XCTAssertEqual(result.indexSummary.marlinEventCount, 1)
        XCTAssertEqual(result.studioStatus.renderLabel, "render packaged")
        XCTAssertEqual(result.studioStatus.scoreLabel, "9/9")
