...
final class ProjectStudioReadinessStatusTests: XCTestCase {
    func testStatusReportsIngestNeededForEmptyProject() throws {
        ...
        XCTAssertEqual(status.primaryAction?.id, "material-rag")
    }
    ...
    func testStatusLabelsPlanningCapabilityAsCandidateExtractionBeforeSelects() throws {
        let (root, project) = try temporaryStudioProject("videoos-studio-planning-triage")
        try writePlanningReadinessFixture(
            root: root,
            project: project,
            hasSelects: false,
            hasBlueprint: false
        ...
        let status = ProjectStudioReadinessStatusReader.status(repositoryRoot: root, projectURL: project)
        let planning = try XCTUnwrap(status.capability("planning"))

        XCTAssertEqual(planning.title, "候補抽出")
        XCTAssertEqual(planning.readinessLabel, "ready for triage")
        XCTAssertEqual(planning.nextCommand, "swift run videoos-studio-cli agent-prompt demo triage")
        XCTAssertEqual(status.actionQueue.first { $0.id == "planning" }?.title, "候補抽出")
        XCTAssertEqual(status.primaryAction?.id, "planning")
    }

    func testStatusRoutesDialoguePlanningBeforeSelectsToAudioEvidence() throws {
        let (root, project) = try temporaryStudioProject("videoos-studio-planning-dialogue-evidence")
        try writeDialoguePlanningMissingAudioFixture(root: root, project: project)

        let status = ProjectStudioReadinessStatusReader.status(repositoryRoot: root, projectURL: project)
        let planning = try XCTUnwrap(status.capability("planning"))

        XCTAssertEqual(planning.title, "音声根拠")
        XCTAssertEqual(planning.readinessLabel, "dialogue evidence missing")
        XCTAssertFalse(planning.isReady)
        XCTAssertEqual(planning.nextCommand, "swift run videoos-studio-cli audio-story-run demo")
        XCTAssertEqual(status.actionQueue.first { $0.id == "planning" }?.title, "音声根拠")
    }

    func testStatusLabelsPlanningCapabilityAsBlueprintAfterSelects() throws {
        let (root, project) = try temporaryStudioProject("videoos-studio-planning-blueprint")
        try writePlanningReadinessFixture(
            root: root,
            project: project,
            hasSelects: true,
        ...
        XCTAssertEqual(planning.title, "構成案")
        XCTAssertEqual(planning.readinessLabel, "ready for blueprint")
        XCTAssertEqual(planning.nextCommand, "swift run videoos-studio-cli agent-prompt demo blueprint")
        XCTAssertEqual(status.actionQueue.first { $0.id == "planning" }?.title, "構成案")
    }

    func testStatusLabelsStaleBlueprintAsBlueprintAction() throws {
        let (root, project) = try temporaryStudioProject("videoos-studio-planning-stale-blueprint")
        try writePlanningReadinessFixture(
            root: root,
            project: project,
            hasSelects: true,
        ...
        XCTAssertEqual(planning.title, "構成案")
        XCTAssertEqual(planning.readinessLabel, "blueprint stale")
        XCTAssertFalse(planning.isReady)
        XCTAssertEqual(planning.nextCommand, "swift run videoos-studio-cli agent-prompt demo blueprint")
        XCTAssertEqual(status.actionQueue.first { $0.id == "planning" }?.title, "構成案")
    }
    ...
    func testStatusReportsStudioReadyWhenApprovedAndRunnable() throws {
        ...
        XCTAssertEqual(status.capability("planning")?.title, "計画")
...
private func writePlanningReadinessFixture(
    root: URL,
    project: URL,
    hasSelects: Bool,
    hasBlueprint: Bool
...
private func writeDialoguePlanningMissingAudioFixture(root: URL, project: URL) throws {
    try "{}".write(to: root.appendingPathComponent("package.json"), atomically: true, encoding: .utf8)
...
private func setPlanningFixtureDate(_ date: Date, project: URL, relativePath: String) throws {
    try FileManager.default.setAttributes(
        [.modificationDate: date],
        ofItemAtPath: project.appendingPathComponent(relativePath).path
    )
}
