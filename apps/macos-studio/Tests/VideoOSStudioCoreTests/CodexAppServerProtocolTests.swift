...
final class CodexAppServerProtocolTests: XCTestCase {
    ...
    func testStudioAgentSurfacePublishingSuppressesIdenticalSurface() {
        XCTAssertFalse(StudioAgentSurfacePublishing.shouldPublish(previous: .review, next: .review))
        XCTAssertFalse(StudioAgentSurfacePublishing.shouldPublish(previous: .ingest, next: .ingest))
        XCTAssertTrue(StudioAgentSurfacePublishing.shouldPublish(previous: .ingest, next: .review))
        XCTAssertTrue(StudioAgentSurfacePublishing.shouldPublish(previous: .compile, next: .package))
    }
