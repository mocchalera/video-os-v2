...
final class ProjectAnalysisRunnerTests: XCTestCase {
    ...
    func testNativeLocalDefaultsAvoidHeavyExternalStagesButKeepMediaLinks() throws {
        ...
        XCTAssertTrue(plan.commandArguments.contains("--skip-appraiser"))
        XCTAssertFalse(plan.commandArguments.contains("--skip-media-link"))
    ...
    func testSpeechLedDefaultsRequireTranscriptionAndKeepOptionalVisualModelsDegraded() throws {
        let (root, project) = try temporaryAnalysisProject("videoos-analysis-speech-led")
        try Data([0]).write(to: project.appendingPathComponent("02_media/source/interview.mp4"))

        let plan = ProjectAnalysisRunPlanner.plan(
            repositoryRoot: root,
            projectURL: project,
            options: .speechLedHighlightDefaults
        )

        XCTAssertTrue(plan.canRun)
        XCTAssertFalse(plan.commandArguments.contains("--skip-stt"))
        XCTAssertTrue(plan.commandArguments.contains("--skip-vlm"))
        XCTAssertTrue(plan.commandArguments.contains("--skip-diarize"))
        XCTAssertFalse(plan.commandArguments.contains("--skip-peak"))
        XCTAssertTrue(plan.commandArguments.contains("--skip-marlin"))
        XCTAssertTrue(plan.commandArguments.contains("--skip-appraiser"))
        XCTAssertFalse(plan.commandArguments.contains("--skip-media-link"))
        XCTAssertFalse(plan.commandArguments.contains("--no-cache"))
        XCTAssertEqual(optionValue(after: "--language", in: plan.commandArguments), "ja")
        XCTAssertEqual(
            optionValue(after: "--content-hint", in: plan.commandArguments),
            "Japanese interview, seminar, or event dialogue for a 60-180 second speech-led highlight"
        )
    }
