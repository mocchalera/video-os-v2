...
final class CandidateBrowserDataSourceTests: XCTestCase {
    ...
    func testCandidateReplacementOperationUsesCandidateReference() {
        ...
        guard case let .replaceSegment(targetClipID, segmentID, candidateRef, sourceInUS, sourceOutUS, reason) = op else {
    ...
    func testCandidateReplacementOperationFallsBackToSegmentReferenceAndTruncatesReason() {
        ...
        guard case let .replaceSegment(_, segmentID, candidateRef, sourceInUS, sourceOutUS, reason) = op else {
            return XCTFail("Expected replace_segment")
        }
