...
function main(): void {
  try {
    ...
    if (
      error instanceof CockpitAskAuthenticatedSurfaceUnavailableError
      || error instanceof CockpitAskResolvedAtUnavailableError
    ) {
      process.stderr.write(canonicalJsonBytes(error.blocker).toString("utf8"));
    } else {
