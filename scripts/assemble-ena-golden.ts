...
function buildBeats(beats: BeatSpec[], resolved: ResolvedClip[], targetDurationSec: number): Array<Record<string, unknown>> {
  const totalClipDurationSec = resolved.reduce((sum, item) => sum + item.clip.duration_s, 0);
  return beats.map((beat) => {
    const items = resolvedForBeat(beat, resolved);
    const beatClipDurationSec = items.reduce((sum, item) => sum + item.clip.duration_s, 0);
    const durationShare = totalClipDurationSec > 0 ? beatClipDurationSec / totalClipDurationSec : 1 / beats.length;
    const targetDurationFrames = Math.max(1, Math.round(targetDurationSec * durationShare * OUTPUT_FPS));
    const refs = items.map((item) => item.candidate.segment_id);
...
function buildBlueprint(projectId: string, beats: BeatSpec[], resolved: ResolvedClip[], exportDurationSec: number): Record<string, unknown> {
  const totalClipDurationSec = roundSeconds(resolved.reduce((sum, item) => sum + item.clip.duration_s, 0));
  const targetDurationSec = roundSeconds(exportDurationSec > 0 ? exportDurationSec : totalClipDurationSec);
  ...
  return {
    ...
    sequence_goals: [
      ...
      "hold the overall PV target near the decoded export duration while scoring only real video clips",
    ],
    beats: buildBeats(beats, resolved, targetDurationSec),
    pacing: {
...
function main(): void {
  ...
  const projectId = analysis.assetsDoc?.project_id ?? analysis.segmentsDoc?.project_id ?? PROJECT_ID;
  ...
  const blueprint = buildBlueprint(projectId, beats, resolved, timelineDoc.export_duration_s ?? 0);
