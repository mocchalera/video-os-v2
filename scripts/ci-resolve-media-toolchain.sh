...
ffmpeg_banner="$("${workdir}/bin/ffmpeg" -version | sed -n '1p')"
ffprobe_banner="$("${workdir}/bin/ffprobe" -version | sed -n '1p')"
case "${ffmpeg_banner}" in
  *"n8.1.2"*) ;;
  *) fail_closed "unexpected ffmpeg banner: ${ffmpeg_banner}" ;;
esac
case "${ffprobe_banner}" in
  *"n8.1.2"*) ;;
  *) fail_closed "unexpected ffprobe banner: ${ffprobe_banner}" ;;
esac
...
if [ -n "${GITHUB_ENV:-}" ]; then
  echo "VIDEOOS_PINNED_FFMPEG=${workdir}/bin/ffmpeg" >> "${GITHUB_ENV}"
  echo "VIDEOOS_PINNED_FFPROBE=${workdir}/bin/ffprobe" >> "${GITHUB_ENV}"
fi

echo "Pinned media toolchain ready: ${PINNED_FFMPEG_VERSION}"
echo "ffmpeg=${workdir}/bin/ffmpeg"
echo "ffprobe=${workdir}/bin/ffprobe"
echo "${ffmpeg_banner}"
echo "${ffprobe_banner}"
echo "sha256=${PINNED_FFMPEG_SHA256}"
