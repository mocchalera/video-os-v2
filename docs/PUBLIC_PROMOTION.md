...
- Stage B remains operationally blocked until a custodian supplies both an
  external Stage-A public signing key and an external approval public signing
  key. The corresponding public key/digest/`key_id` values are reviewed and
  committed in `runtime/release/public-promotion-trust.json`. No private key may
  enter this repository. Neither `stage_a_authentication` nor
  `approval_authentication` is configured yet.
- Production approval authority is an external detached signature over the
  exact canonical `external-signed-promotion-approval/v1` bytes. That artifact
  binds Stage A SHA, the fixed destination, `push-exact-projection` /
  `push` / workflow scope, a human approver identity, a calendar-valid RFC 3339
  `approved_at`, and a stable `approval_id`. Cockpit itself is not modified and
  is not required for production promotion.
- The legacy Cockpit Ask adapter stays fail-closed. `cockpit task get`
  conversation text is unauthenticated, even when it contains hand-written v2
  JSON, a fake `resolved_at`, or `kind`/`source` labels. Recording through
  `scripts/record-public-promotion-approval.ts` therefore emits
  `cockpit-ask-authenticated-surface-unavailable/v1` and does not write a
  receipt. That command is not a fallback for the signed route.
...
- a trusted approval artifact binding Stage A SHA, destination, branch, exact
  `push-exact-projection` operation/event/workflow scope, human identity, and
  approval time. Production verifies only
  `external-signed-promotion-approval/v1` through `detached-signature` against
  the configured approval public key. The v1 Cockpit receipt contract
  (`promotion-approval-receipt/v1` + `cockpit_source` +
  `cockpit-approval-event`) remains valid for test-only injection and is not
  the production authority. Approval time uses the supported RFC 3339 subset
  with calendar-valid month/day/leap-year values, seconds `00` through `59`,
  and a valid offset; leap-second `:60` values are intentionally unsupported
  by both the runtime and schema.
...
branch, workflow, provider repository ID, and public-key trust roots are loaded
from repository configuration, never CLI arguments. Production verify requires
both `--stage-a-signature` and `--approval-signature`. It verifies those
detached signatures with OpenSSL, re-fetches the exact GitHub run/jobs, and
then re-applies the exact root-commit and tree contract. Unsigned caller JSON,
self-supplied trust roots, conversation text, and manual booleans are never
accepted as Stage A, CI, or approval evidence. Unconfigured Stage A trust
prints `stage-a-signature-handoff/v1`. Unconfigured approval trust prints
`approval-signature-handoff/v1`. Neither writes an envelope.
...
2. After CI completes, run `scripts/collect-public-ci-evidence.ts`; it performs
   ...
   public repository.
3. Production approval uses the signed route only. Run
   `scripts/prepare-public-promotion-approval.ts` to emit the canonical request,
   then `scripts/prepare-signed-public-promotion-approval.ts` to emit the exact
   `external-signed-promotion-approval/v1` bytes the custodian must sign.
   After the custodian returns a detached signature over those bytes, run
   `scripts/record-signed-public-promotion-approval.ts` with `--request`,
   `--approval`, and `--signature`. Unconfigured `approval_authentication`
   prints `approval-signature-handoff/v1` and does not write a receipt.
   `scripts/record-public-promotion-approval.ts` remains the separate Cockpit
   recorder and never falls back to, or accepts, the signed artifact.
4. After an external custodian supplies public keys, digests, key IDs, and
   detached signatures over the exact Stage-A bytes and the exact approval
   bytes — never repository-generated private keys — configure
   `stage_a_authentication` and `approval_authentication`, then run
   `scripts/verify-promotion-envelope.ts` with both signatures, the provider
   receipt, the signed approval receipt, the exact local public commit, and
   the private envelope output. Unconfigured Stage A trust prints
   `stage-a-signature-handoff/v1`. Unconfigured approval trust prints
   `approval-signature-handoff/v1`. Neither emits a Stage B envelope.
