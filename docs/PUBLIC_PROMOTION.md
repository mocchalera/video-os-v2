...
- The Cockpit adapter requires a provider-authenticated first-party structured
  `cockpit.ask.resolved` v2 event or authenticated execution receipt with
  `outcome: answered`, `answered_by: user`, the exact request-digest choice, and
  calendar-valid `resolved_at`. Production default cannot mint a receipt from
  current Cockpit: `cockpit task get` conversation text is unauthenticated, even
  when it contains hand-written v2 JSON, a fake `resolved_at`, or
  `kind`/`source` labels. `cockpit ask get` / `cockpit ask list` also do not
  expose that surface. Recording therefore fail-closes with typed blocker
  `cockpit-ask-authenticated-surface-unavailable/v1`
  (`cockpit_ask_authenticated_surface_unavailable`). A later Cockpit change must
  supply a typed surface the current CLI can actually read; until then the
  default reader stays fail-closed. Conversation text, a hand-written
  timestamp, wall clock, filesystem mtime, or conversation order is not
  accepted.
...
3. Run `scripts/prepare-public-promotion-approval.ts`, use its exact printed
   ...
   Recording fails if the durable event lacks the exact user answer. The
   production default also cannot mint a receipt from current Cockpit
   conversation text, including hand-written v2 JSON; it emits
   `cockpit-ask-authenticated-surface-unavailable/v1` and does not write a
   receipt.
4. After an external custodian supplies a public key, digest, key ID, and a
   detached signature over the exact Stage-A bytes — never a repository-generated
   private key — configure `stage_a_authentication` and run
   ...
   output. Unconfigured trust prints `stage-a-signature-handoff/v1` and does
   not emit a Stage B envelope.
