...
- 旧 Script Engine 側はこの問題を `candidate_id` / `material_unit_id` で解いていました。`/path/to/legacy-project/docs/inventory-ja/rules/narrative-design.md:131-179`
...
- 旧 skill の中には、明示的に audio overlap や multi-clip compression や interleave を要求するものがあります。`/path/to/legacy-project/docs/inventory-ja/editing-skills/j-cut-lead-in.md:41-51` `/path/to/legacy-project/docs/inventory-ja/editing-skills/l-cut-reaction-hold.md:42-54` `/path/to/legacy-project/docs/inventory-ja/editing-skills/montage-compress.md:24-52` `/path/to/legacy-project/docs/inventory-ja/editing-skills/crosscut-suspense.md:24-51`
...
- 旧 profile の差分は `platform`, `aspect_ratio`, `caption`, `qa_policy`, `narrative_lock` など複数面にまたがっています。`/path/to/legacy-project/docs/inventory-ja/profiles/interview-highlight.md:19-122` `/path/to/legacy-project/docs/inventory-ja/profiles/lp-testimonial.md:20-61`
...
- 旧 `quality-judgment` は LLM ベース審査で、ここを deterministic に落とす具体 proxy はまだ書かれていません。`/path/to/legacy-project/docs/inventory-ja/skills/quality-judgment.md:5-44`
