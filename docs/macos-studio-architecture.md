...
2. ~~Run a real local Marlin-2B evaluation on representative interview and music-video footage, then use `marlin-status` coverage and segment-peak evidence before flipping it from opt-in to preferred default.~~ Done (2026-06-12): live Marlin-2B (MPS) evaluated real footage on rokutaro-bicycle-growth-20260427 (18/55 segments, coverage 0.33) and fumoto-growth (11/32, coverage 0.34); both are now preference candidates alongside the two smoke projects (repo: 4 candidates, 235 events, aggregate coverage 0.36, all three representative buckets covered). Real footage exposed and fixed three field bugs on the way: the studio subprocess pipe deadlock (SubprocessRunner), unbounded worker memory (marlin-proxy bounded proxies), and overwrite-only artifacts (incremental merge in the marlin stage). Long clips still cost minutes per request on MPS — evaluate in bounded subsets via incremental merge rather than whole projects at once.
