...
describe("editorial craft directives", () => {
  ...
  it("ignores Marlin trim plans outside the selected candidate source range", () => {
    const candidate = makeCandidate({
      src_in_us: 6_000_000,
    ...
    const blueprint = { trim_policy: { mode: "adaptive" } } as EditBlueprint;
    const beat = makeBeat("B01", { target_duration_frames: 48 });

    const result = applyAdaptiveTrim(
      [clip],
      [candidate],
      blueprint,
      [beat],
      ...
      [
        {
          ...
          rationale: "source-start Marlin event",
          technique: "peak_hold",
          source: "marlin_event" as const,
          event_id: "MEV_SOURCE_START",
          score: 0.9,
        },
      ],
    );

    expect(result.get(clip.clip_id)?.mode).not.toBe("clip_trim_plan");
    expect(clip.src_in_us).toBe(6_000_000);
    expect(clip.src_out_us).toBe(8_000_000);
    expect(clip.metadata?.trim).not.toMatchObject({ clip_trim_source: "marlin_event" });
  });
