...
describe("Compiler micro-clip guard", () => {
  ...
  it("drops unintentional V1 micro-clips, removes generated audio mirrors, and closes the gap", () => {
    ...
    expect(logs).toEqual([
      "[compile] dropped 1 micro-clip(s) below minimum renderable duration: C_MICRO",
    ]);
  });
  ...
  it("preserves authored short placements under human_golden_order", () => {
    const authored = makeClip({
      clip_id: "C_AUTHORED",
      segment_id: "S_AUTHORED",
      candidate_ref: "cand_authored",
      timeline_in_frame: 0,
      timeline_duration_frames: 11,
    });
    const timeline = makeTimeline([authored]);

    const result = dropUnintentionalMicroClips(timeline, {
      preserveAuthoredCandidatePlan: true,
      log: () => undefined,
    });

    expect(result.dropped).toBe(0);
    expect(timeline.tracks.video[0].clips).toHaveLength(1);
    expect(timeline.tracks.video[0].clips[0].metadata?.editorial).toEqual({
