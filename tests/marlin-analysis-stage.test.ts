...
describe("Marlin analysis stage", () => {
  ...
  it("does not materialize temporal Marlin peaks onto candidates outside the peak range", () => {
    const projectDir = makeTempProject();
    fs.mkdirSync(path.join(projectDir, "03_analysis"), { recursive: true });
    fs.writeFileSync(
      path.join(projectDir, "03_analysis/segments.json"),
              ...
              peak_moments: [
                {
                  ...
                  type: "emotional_peak",
                  confidence: 0.9,
                  description: "The opening setup moment.",
                  source_pass: "marlin_caption",
                },
              ],
              recommended_in_out: {
                best_in_us: 0,
                best_out_us: 2_000_000,
                rationale: "Marlin temporal semantic event overlapped the source start.",
                source_pass: "marlin_caption",
              },
              support_signals: {
                motion_support_score: 0.9,
                audio_support_score: 0.1,
    ...
    const selects: {
      candidates: Array<{
        ...
        src_out_us: number;
        confidence: number;
        peak_signals?: Record<string, unknown>;
        editorial_signals?: Record<string, unknown>;
        trim_hint?: Record<string, unknown>;
      }>;
    } = {
    ...
    expect(materializePeakSignalsFromSegments(projectDir, selects)).toBe(false);
    expect(selects.candidates[0].peak_signals).toBeUndefined();
    expect(selects.candidates[0].editorial_signals).toBeUndefined();
    expect(selects.candidates[0].trim_hint).toBeUndefined();
  });
